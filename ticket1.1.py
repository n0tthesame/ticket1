...
# ==================== Обработка звукового файла с исключениями ====================
try:
    # Проверяем существование файла
    file_path = "../data/ml_1.mp3"
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"Звуковой файл не найден: {file_path}")
    
    # Загружаем звуковой файл
    data, sr = librosa.load(file_path, sr=None)
    print(f"Звуковой файл успешно загружен: {file_path}")
    
except FileNotFoundError as e:
    print(f"Ошибка: {e}")
    print("Создаем заглушку для звуковых данных...")
    # Создаем заглушку для работы программы
    data = None
    sr = 22050
    
except Exception as e:
    print(f"Неожиданная ошибка при загрузке звукового файла: {e}")
    print("Создаем заглушку для звуковых данных...")
    data = None
    sr = 22050
    ...
    def make_sound(self) -> str:
        try:
            if data is not None:
                ...
                return "Быстрее! Быстрее! (звуковой файл недоступен)"
        except Exception as e:
            print(f"Ошибка воспроизведения звука: {e}")
            return "Быстрее! Быстрее! (ошибка воспроизведения звука)"
...
def main():
    ...
    try:
        ...
        print("\nСОЗДАНИЕ МОБОВ ЧЕРЕЗ ЛЕСНУЮ ФАБРИКУ")
        ...
        print(f"Скелет говорит: {skelet.make_sound()}")
        print(f"Скелет атакует: {skelet.attack()}")
        
        print("\nСОЗДАНИЕ МОБОВ ЧЕРЕЗ ФАБРИКУ ПОЛЯНЫ")
        
        # Демонстрируем мобов с поляны
        meadow_spawner = MobSpawner(meadow_factory)
        mellstroy_mob = meadow_spawner.spawn_neutral_mob()
        
        print(f"\nСоздан Мелстрой: {mellstroy_mob.display()}")
        print(f"Мелстрой говорит: {mellstroy_mob.make_sound()}")
        print(f"Мелстрой атакует: {mellstroy_mob.attack()}")
        
        crunpel = meadow_spawner.spawn_hostile_mob()
        print(f"\nСоздан Крипер: {crunpel.display()}")
        print(f"Крипер говорит: {crunpel.make_sound()}")
        print(f"Крипер атакует: {crunpel.attack()}")
